...
class NotebookManager:
    ...
    def add(self, key: str, value: str) -> tuple[bool, str]:
        ...
        if current_size + new_size > self.MAX_TOTAL_SIZE:
            return False, f"Notebook size limit reached ({self.MAX_TOTAL_SIZE} chars max)"
            ...
            if line.startswith('## '):
                ...
                if current_key and current_value:
                    ...
                    if value_text and not value_text.startswith('_Created:'):
                        success, _ = self.add(current_key, value_text)  # Uses MARM validation
                        if success:
                            imported_count += 1
                ...
                current_value = []
            elif current_key and not line.startswith('#') and not line.startswith('_Created:'):
...
def list_entries():
    ...
    click.echo(f"📔 Notebook Entries ({len(notebook)}/{NotebookManager.MAX_ENTRIES}):\n")
